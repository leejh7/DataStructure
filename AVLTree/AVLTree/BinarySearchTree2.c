...
{
	if (*pRoot == NULL)		// 추가되야 할 노드의 위치
	{
		*pRoot = MakeBTreeNode();
		SetData(*pRoot, data);
	}
	else if (data < GetData(*pRoot))	// 조건에 의해 왼쪽 노드에 Insert 되야하는 경우
	{
		BSTInsert(&(*pRoot)->left, data);
		*pRoot = Rebalance(pRoot);		// 위의 코드로 새로운 노드가 추가된 후 위로 올라오면서 Rebalancing
	}
	else if (data > GetData(*pRoot))	// 조건에 의해 오른쪽 노드에 Insert 되야하는 경우
	{
